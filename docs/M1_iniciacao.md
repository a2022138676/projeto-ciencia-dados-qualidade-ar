...
O dataset selecionado, **AirQualityUCI**, contém medições **horárias** de poluentes atmosféricos (por exemplo, `CO(GT)`, `NOx(GT)` e `NO2(GT)`), leituras de sensores químicos (`PT08.*`) e variáveis meteorológicas (temperatura, humidade relativa e humidade absoluta). Como o dataset não inclui uma variável única chamada “qualidade do ar”, a qualidade do ar será representada por um **indicador mensurável** presente nos dados. Para a abordagem de **aprendizagem supervisionada (regressão)**, a **variável-alvo** definida é **`CO(GT)`**, por ser uma medida *ground truth* de poluente.
...
- É possível prever `CO(GT)` com desempenho superior a um baseline simples (por exemplo, média histórica), utilizando modelos de regressão?

## 2. Objetivos SMART
*Defina os objetivos do projeto seguindo a lógica SMART (Específico, Mensurável, Atingível, Relevante e Temporal):*

1. **Objetivo 1:** Até ao final da **Milestone 2 (24/03/2026)**, preparar e explorar o dataset **AirQualityUCI** de forma reprodutível, incluindo: carregamento correto do CSV (`sep=';'`, `decimal=','`), remoção de colunas/linhas totalmente vazias, conversão de `Date` + `Time` para `timestamp` e substituição do código de valores em falta `-200` por `NaN`.  
   **Métrica de sucesso:** geração de um relatório/tabela com a **percentagem de valores em falta por variável** e notebook de EDA com visualizações e conclusões registadas.

2. **Objetivo 2:** Até ao final da **Milestone 3 (21/04/2026)**, desenvolver e comparar **pelo menos 3 modelos de regressão** para prever a variável-alvo **`CO(GT)`**, avaliando com **MAE** e **RMSE** (e **R²** como métrica complementar), e comparando com um **baseline** simples (ex.: média histórica).  
   **Métrica de sucesso:** obter uma melhoria mínima de **15% em MAE ou RMSE** face ao baseline.

## 3. Metodologia de Gestão (PBL)
* **Divisão de Tarefas:**
  * **Rodrigo Pedro:** Responsável pela **Engenharia de Dados** (importação do dataset no Kaggle, carregamento do CSV com parsing correto, limpeza estrutural, conversão `-200 → NaN`, criação de `timestamp` e organização do repositório).
  * **Tiago Rodrigues:** Responsável pela **Modelação Estatística / EDA e Documentação** (análise exploratória, gráficos e conclusões, correlações, preparação para modelação, atualização do `README.md` e documentação em `docs/`).
...
* **Disponibilidade:** O dataset foi obtido e utilizado em dois formatos (`AirQualityUCI.csv` e `AirQualityUCI.xlsx`) e importado para um **Notebook no Kaggle**. No Kaggle, o CSV foi lido com `sep=';'` e `decimal=','`, devido ao formato do ficheiro.
* **Qualidade Inicial:** Foram executados `df.head()`, `df.info()` e `df.describe()` para inspeção inicial. Após remoção de colunas/linhas totalmente vazias, o dataset apresenta **9357 linhas** e **15 colunas úteis**. Verificou-se que:
